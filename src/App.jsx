...
export default function App() {
  ...
  const changeSection = (next) => {
    ...
    setSort('random')
    setPage(1)
    // totalCount مال بخش قبلی بود (مثلاً کل آرشیو یا یه بخش دیگه) — تا
    // جواب fetch جدید نرسه، اگه پاک نشه همون عدد اشتباه/قدیمی زیر عنوان
    // بخش جدید لحظه‌ای نشون داده می‌شه (مثلاً «۱۷۶۷۴ physical films»).
    setTotalCount(null)
    // films هم مال بخش قبلی بود — چون Header وقتی totalCount هنوز null
    // باشه به sectionFilms.length برمی‌گرده، بدون پاک‌کردنش یه لحظه طول
    // صفحه‌ی قبلی (مثلاً همیشه ۴۸، یعنی PAGE_SIZE) به‌جای عدد درستِ بخش
    // جدید نشون داده می‌شد.
    setFilms([])
